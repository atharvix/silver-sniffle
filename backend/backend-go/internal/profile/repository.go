...
func (r *PostgresRepository) Upsert(ctx context.Context, p *domain.Profile) error {
	...
	txErr := r.db.WithTx(ctx, func(tx pgx.Tx) error {
		profileQuery := `
			INSERT INTO profiles (email, name, bio, photo_url, latitude, longitude, last_seen_at, updated_at, email_verified)
			VALUES ($1, $2, $3, $4, $5::double precision, $6::double precision, CASE WHEN $5::double precision IS NOT NULL AND $6::double precision IS NOT NULL THEN $7::timestamptz ELSE NULL END, $7::timestamptz, TRUE)
